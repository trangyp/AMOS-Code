...
class BranchFieldEngine:
    ...
    def generate(self, current_state: Dict, workspace: GlobalWorkspace, 
                 n_branches: int = 3) -> List[Branch]:
        """
    ...
    def generate_forks(self, state: Dict, context: Dict, n: int = 3) -> List[Branch]:
        ...
        workspace = MockWorkspace(Goals=[{'description': context.get('goal', 'default'), 'priority': 0.5}])
...
class CollapseEngine:
    ...
    def can_commit(self, branch: Branch) -> bool:
        ...
        """
        return (
            branch.confidence >= self.tau1_confidence and
            branch.risk <= self.tau3_risk
        )
...
class MorphExecutor:
    ...
    def _apply_step(self, step: Dict) -> Dict:
        """Apply a step."""
        return {'step': step, 'status': 'applied', 'timestamp': datetime.utcnow().isoformat()}
...
class AMOSKernel:
    ...
    def cycle(self) -> Dict:
        ...
        # 2. Update Workspace
        signals = [{'id': f'sig_{i}', 'priority': 0.5, 'risk': 0.2, 
                   'novelty': 0.3, 'goal_relevance': 0.6, 
                   'temporal_urgency': 0.4} for i in range(3)]
        self.workspace.update(signals)
        print(f"2. Workspace: Focus = {self.workspace.Focus}")
        
        # 3. Generate Branches
        branches = self.branch_engine.generate(
            self.state.U_t, self.workspace, n_branches=3
        )
        print(f"3. Generated: {len(branches)} branches")
        
        # 4. Simulate (already done in generation)
        print(f"4. Simulated: All branches scored")
        ...
        if selected and self.collapse_engine.can_commit(selected):
            ...
            if result['success']:
                ...
                print(f"9-11. Learn: State updated")
        else:
            print(f"7. Commit: ✗ Rejected - insufficient confidence")
    ...
    def run(self, n_cycles: int = 3):
        ...
        print(f"Final State: {len(self.universal_graph.V)} nodes, {len(self.workspace.Goals)} goals")
        print("="*60)
