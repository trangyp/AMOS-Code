...
class AMOSOperational:
    ...
    def _update_health(self, cycle_result: Dict):
        ...
        for decision in cycle_result.get('decisions', []):
            ...
            self.health_status['identity_drift'] = max(
                self.health_status['identity_drift'],
                drift
            )
        ...
        if cycle_result['status'] == 'success':
            self.health_status['decision_quality'] = min(
                1.0,
                self.health_status['decision_quality'] + 0.01
            )
        else:
        ...
        self.health_status['system_stable'] = (
            self.health_status['identity_drift'] < 0.3 and
            self.health_status['decision_quality'] > 0.5
        )
    ...
    def run_continuous(self, interval_minutes: int = 60):
        ...
        while self.running:
            try:
                ...
                if self.connectors:
                    self.connectors.notifications.notify(
                        f"Critical error in cycle: {e}", 'error'
                    )
    ...
    def _recovery_procedure(self):
        ...
        if self.connectors:
            self.connectors.notifications.notify(
                "Recovery procedure completed", 'info'
            )
