...
class AMOSWorkflows:
    ...
    def workflow_code_analysis(self) -> WorkflowResult:
        ...
        analysis = amos.analyze_with_rules("Analyze codebase for complexity and optimization opportunities")
        steps += 1
...
"""
        ...
        output_lines = [
            ...
            f"- Files analyzed: See execution output",
            "- Status: Complete"
        ]
        output = "\\n".join(output_lines)
        steps += 1
        ...
        return WorkflowResult(
            ...
            resources_used={"brain_analysis": True, "subsystems": len(routing.get('subsystems', []))}
        )
    ...
    def workflow_health_check(self) -> WorkflowResult:
        ...
        print(f"    ✓ Subsystems: {orch_health.get('subsystems_online', 0)}/{orch_health.get('subsystems_total', 14)}")
        ...
        all_healthy = (
            brain_status.get('engines_count', 0) >= 12 and
            orch_health.get('subsystems_online', 0) >= 7
        )
        steps += 1
    ...
    def print_summary(self):
        ...
        for r in self.results:
            ...
            print(f"  {icon} {r.workflow_name}: {r.steps_completed}/{r.steps_total} steps ({r.execution_time_ms}ms)")
