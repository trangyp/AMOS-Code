...
from amosl_bridge import BridgeExecutor, SubstrateType, BridgeResult
...
class EvolutionOperator:
    ...
    def _op_bridge(self, state: dict[str, Any]) -> dict[str, Any]:
        ...
        # Execute bridge
        result = self.bridge.execute_cross_substrate(
            source, target, "evolution_step", state
        )
    ...
    def create_evolution_chain(
        ...
        substrate_path: Optional[list[SubstrateType]] = None
    ) -> EvolutionChain:
        """
        ...
        Returns:
            New evolution chain
        """
        chain_id = f"EVO-{datetime.utcnow().strftime('%Y%m%d%H%M%S')}"
...
def demo_evolution():
    ...
    chain = evo.create_evolution_chain(
        initial_state,
        substrate_path=[SubstrateType.CLASSICAL, SubstrateType.BIOLOGICAL]
    )
    print(f"    Chain ID: {chain.chain_id}")
    print(f"    Initial state: Σ_0")
    ...
    step2 = evo.evolve(
        chain.chain_id,
        operator="bridge",
        target_substrate=SubstrateType.BIOLOGICAL
    )
