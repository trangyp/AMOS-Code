...
AMOS_TOOL_SCHEMA = {
    ...
    "input_schema": {
        ...
        "properties": {
            "action": {
                "type": "string",
                "enum": ["think", "analyze_task", "design_architecture", "audit_decision", "status"],
                "description": (
            ...
            "mode": {
                "type": "string",
                "enum": ["exploratory_mapping", "diagnostic_analysis", "design_and_architecture", 
                         "audit_and_critique", "measurement_and_scoring"],
...
def _amos_tool(params: dict, config: dict) -> str:
    ...
    if action == "think":
        ...
        if not decision or not rationale:
            return "[AMOS Error] Both 'decision' and 'rationale' required for 'audit_decision' action"
