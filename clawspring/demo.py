...
def demo():
    ...
    print("=" * 60)
    state4 = AgentState()
    _run_demo(state4, config, system_prompt, "What are the tools available to you?")
    _run_demo(
        state4,
        config,
        system_prompt,
...
def _run_demo(state: AgentState, config: dict, system_prompt: str, prompt: str):
    ...
    for event in run(prompt, state, config, system_prompt):
        if isinstance(event, TextChunk):
            ...
            print(
                f"\n\033[36m  ⚙ {event.name}({list(event.inputs.values())[0] if event.inputs else ''})\033[0m",
                flush=True,
            )
            ...
            print(
                f"\n\033[2m  [+{event.input_tokens} in / +{event.output_tokens} out]\033[0m",
                flush=True,
            )
