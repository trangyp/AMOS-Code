...
def draw_phone(img, chat_messages):
    ...
    for sender, text, _color in chat_messages:
        ...
        d.rounded_rectangle(
            [bx, msg_y, bx + bubble_w, msg_y + bubble_h], radius=10, fill=bubble_color
        )
...
BANNER_TG = [
    ...
    [
        seg("│  ", SUBTEXT),
        seg("Permissions: ", SUBTEXT),
        seg("auto", YELLOW, True),
        seg("  flags: [", SUBTEXT),
        seg("telegram", TEAL, True),
        seg("]", SUBTEXT),
    ],
...
def build_scenes():
    ...
    add(
        tool_done + [text_line(l, 2) if l else None for l in resp1_lines] + [claude_sep()],
        500,
        chat=phone_q1_typing,
    )

    # ── 4: Response sent to Telegram ─────────────────────────────────────
    phone_r1 = phone_q1_typing + [
        (
            "bot",
            "Here are the files in this project: clawspring.py, agent.py, tools.py, providers.py, config.py …",
            GREEN,
        )
    ]
    ...
    after_r1 = (
        ...
        + [text_line(l, 2) if l else None for l in resp1_lines]
        + [claude_sep(), None]
    )
    ...
    add(
        after_r1
        + [tg_sent("Here are the files in this project..."), None, prompt_line(cursor=True)],
        800,
        chat=phone_r1,
    )
    ...
    add(
        ...
        + [
            ...
            [
                seg("  📩 Telegram: ", TEAL, True),
                seg("/cost", TEXT),
                seg("  (slash command passthrough)", SUBTEXT),
            ],
        ],
        900,
        chat=phone_q2,
    ...
    phone_cost = phone_q2 + [
        ("bot", "Input: 3,241 tokens | Output: 487 tokens | Cost: $0.0521 USD", GREEN)
    ]
    add(
        cost_base + cost_lines + [None, tg_sent("Input: 3,241 tokens | Output: 487 …")],
        900,
        chat=phone_cost,
    )
    add(
        cost_base
        + cost_lines
        + [None, tg_sent("Input: 3,241 tokens | Output: 487 …"), None, prompt_line(cursor=True)],
        800,
        chat=phone_cost,
    )
    ...
    phone_r3 = phone_q3 + [
        (
            "bot",
            "/brainstorm starts a multi-persona AI debate and synthesizes a Master Plan…",
            GREEN,
        )
    ]
