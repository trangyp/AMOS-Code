...
def cmd_amos(args: str, state, config) -> bool:
    ...
    if not args:
        ...
        if status["enabled"]:
            ok("AMOS Brain: ACTIVE")
            info(f"  System: {status.get('system_name', 'Unknown')}")
            info(f"  Version: {status.get('version', 'Unknown')}")
            info(f"  Engines: {status.get('engines_loaded', 0)}")
            info(f"  Laws: {status.get('laws_loaded', 0)}")
    ...
    if args in ("on", "enable", "true", "1"):
        ...
        if not status["enabled"]:
            err(f"Cannot enable AMOS mode: {status.get('error', 'Brain not loaded')}")
            return True
        config["_amos_mode"] = True
        ok("AMOS Brain cognitive mode: ON")
        info("Next query will use AMOS cognitive architecture")
        return True

    # Disable AMOS mode
    if args in ("off", "disable", "false", "0"):
        config["_amos_mode"] = False
        ok("AMOS Brain cognitive mode: OFF")
...
def repl(config: dict, initial_prompt: str = None):
    ...
    if not initial_prompt:
        ...
        if config.get("_amos_mode"):
            active_flags.append("amos")
        if active_flags:
