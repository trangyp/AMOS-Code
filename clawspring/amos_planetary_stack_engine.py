...
class SustainabilityMetricsKernel:
    ...
    def calculate_overshoot_day(self) -> dict[str, Any]:
        ...
        if self.footprint_per_capita > self.biocapacity_per_capita:
            ...
            return {
                ...
                "biocapacity_deficit": round(
                    self.footprint_per_capita - self.biocapacity_per_capita, 2
                ),
                "num_earths_needed": round(
                    self.footprint_per_capita / self.biocapacity_per_capita, 2
                ),
            }
    ...
    def assess_doughnut_economics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "in_doughnut": all(v > 0.8 for v in social_foundation.values())
            and all(v < 0.8 for v in planetary_ceiling.values()),
        }
...
class PlanetaryBoundariesKernel:
    ...
    def __init__(self):
        self.boundaries: dict[str, dict] = {
            ...
            PlanetaryBoundary.BIODIVERSITY_LOSS.value: {
                "current": 100,
                "safe": 10,
                "unit": "E/MSY",
            },
            PlanetaryBoundary.LAND_USE_CHANGE.value: {"current": 75, "safe": 75, "unit": "%"},
            PlanetaryBoundary.FRESHWATER_USE.value: {
                "current": 2600,
                "safe": 4000,
                "unit": "km3/yr",
            },
            ...
            PlanetaryBoundary.OCEAN_ACIDIFICATION.value: {
                "current": 8.1,
                "safe": 8.2,
                "unit": "pH",
            },
...
class PlanetaryStackEngine:
    ...
    def analyze(self, query: str, context: dict[str, Any] = None) -> dict[str, Any]:
        ...
        if any(
            kw in query_lower for kw in ["sustainability", "overshoot", "footprint", "doughnut"]
        ):
            focus_areas.append("sustainability")
        if any(
            kw in query_lower for kw in ["boundaries", "planetary limits", "safe operating space"]
        ):
        ...
        climate_health = (
            results.get("climate_analysis", {}).get("stability", {}).get("stability_index", 0.5)
        )
        eco_health = (
            1.0
            - results.get("ecology_analysis", {})
            .get("biodiversity_crisis", {})
            .get("habitat_loss_percent", 50)
            / 100
        )
        ...
        if boundaries_crossed > 3:
            recommendations.append(
                f"URGENT: {boundaries_crossed} planetary boundaries crossed - transformative change needed"
            )
        ...
        if "resources" in focus_areas:
            nitrogen_status = (
                results.get("resources_analysis", {}).get("nitrogen", {}).get("status", "")
            )
            if nitrogen_status == "overshoot":
                recommendations.append(
                    "Nitrogen: Reduce synthetic fertilizer use, improve nutrient recycling"
                )
        if "sustainability" in focus_areas:
            overshoot_data = results.get("sustainability_analysis", {}).get("overshoot", {})
            if "num_earths_needed" in overshoot_data:
                recommendations.append(
                    f"Overshoot: Currently using {overshoot_data['num_earths_needed']} Earths - reduce consumption"
                )
    ...
    def get_findings_summary(self, results: dict) -> str:
        ...
        for name, data in list(status.items())[:5]:
            ...
            lines.append(
                f"- {marker} **{name}**: {data.get('current')} {data.get('unit')} (safe: {data.get('safe_limit')})"
            )
        lines.extend(
            [
                "",
...
# Singleton instance
_planetary_engine: PlanetaryStackEngine | None = None
