...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="AMOS Ecosystem v2.0 - Cognitive Architecture"
    )
    parser.add_argument(
        "--version", "-v",
        action="version",
        version="AMOS Ecosystem v2.0"
    )
    parser.add_argument(
        "--validate",
        action="store_true",
    ...
    parser.add_argument(
        "--orchestrate",
        metavar="TASK",
        help="Run master orchestrator on a task"
    )
    ...
    if args.orchestrate:
        ...
        result = orchestrator.orchestrate_cognitive_task(
            "cli_task", args.orchestrate, "MEDIUM"
        )
