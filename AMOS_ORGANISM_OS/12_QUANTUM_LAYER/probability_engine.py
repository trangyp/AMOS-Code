...
class ProbabilityDistribution:
    ...
    def to_dict(self): return {**asdict(self), "total_prob": sum(self.outcomes.values())}
...
class ProbabilityEngine:
        ...
        if f.exists():
            try:
                ...
                for d in data.get("distributions", []):
                    self.distributions.append(ProbabilityDistribution(
                        id=d["id"], name=d["name"], outcomes=d["outcomes"],
    ...
    def create_distribution(self, name: str, outcomes: Dict[str, float]) -> ProbabilityDistribution:
        ...
        if total > 0: outcomes = {k: v/total for k, v in outcomes.items()}
...
def get_probability_engine(data_dir=None):
    global _ENGINE
    if _ENGINE is None: _ENGINE = ProbabilityEngine(data_dir)
    return _ENGINE
