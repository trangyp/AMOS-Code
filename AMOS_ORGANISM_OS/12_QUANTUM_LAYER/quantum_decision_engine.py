...
class QuantumDecisionEngine:
    ...
    def _load_data(self):
        ...
        if f.exists():
            try: self.decisions = json.loads(f.read_text()).get("decisions", [])
            except: pass
    ...
    def probabilistic_decide(self, paths: List[DecisionPath]) -> DecisionPath:
        ...
        if not paths: return DecisionPath(name="no_options")
...
def get_quantum_decision_engine(data_dir=None):
    global _ENGINE
    if _ENGINE is None: _ENGINE = QuantumDecisionEngine(data_dir)
    return _ENGINE
...
if __name__ == "__main__":
    ...
    paths = [
        DecisionPath(name="Fast", description="Quick solution", expected_value=0.7, uncertainty=0.3),
        DecisionPath(name="Thorough", description="Comprehensive", expected_value=0.9, uncertainty=0.2),
    ]
