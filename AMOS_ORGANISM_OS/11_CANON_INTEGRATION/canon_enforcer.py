...
class CanonEnforcer:
    ...
    def get_violation_report(self) -> ViolationReport:
        ...
        critical = sum(1 for v in self.violations if v.severity == RulePriority.CRITICAL and not v.resolved)
        high = sum(1 for v in self.violations if v.severity == RulePriority.HIGH and not v.resolved)
        medium = sum(1 for v in self.violations if v.severity == RulePriority.MEDIUM and not v.resolved)
