...
class PrimaryLoop:
    ...
    def execute_cycle(self, task: str, context: Optional[Dict] = None) -> CycleResult:
        ...
        for subsystem_code in self.PRIMARY_SEQUENCE:
            ...
            try:
                ...
                if subsystem is None:
                    ...
                    subsystem_results[subsystem_code] = {"status": "skipped", "reason": "not_available"}
                    continue
        ...
        print(f"  Subsystems: {len([r for r in subsystem_results.values() if r.get('status') != 'skipped'])}/{len(self.PRIMARY_SEQUENCE)}")
        print()
    ...
    def _execute_subsystem(self, code: str, subsystem: Any, task: str, context: Optional[Dict]) -> Dict[str, Any]:
