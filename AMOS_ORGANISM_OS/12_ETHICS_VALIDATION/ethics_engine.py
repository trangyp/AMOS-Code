...
class EthicsEngine:
    ...
    def evaluate_decision(self, context: str, action: str, 
                          data: Dict[str, Any]) -> EthicalDecision:
        ...
        approved = score >= 0.7 and not any(
            self.rules[v].severity == EthicsSeverity.CRITICAL 
            for v in violations if v in self.rules
        )
    ...
    def _check_rule(self, rule: EthicsRule, context: str, 
