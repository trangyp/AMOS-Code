...
class AMOSUnifiedRuntime:
    ...
    def _catalog_knowledge(self) -> Dict[str, Any]:
        ...
        if packs_path.exists():
            stats["countries"] = len(list((packs_path / "Country_Packs").glob("*"))) if (packs_path / "Country_Packs").exists() else 0
            stats["sectors"] = len(list((packs_path / "Sector_Packs").glob("*"))) if (packs_path / "Sector_Packs").exists() else 0
    ...
    def status(self) -> Dict[str, Any]:
        ...
        total_systems = len([s for s in self._init_status.values() if "✅" in str(s.get("status", ""))])
