...
class BranchEnergyBudget:
    ...
    def charge_for_simulation(self, n_branches: int) -> bool:
        ...
        sim_consumer = EnergyConsumer(
            consumer_id='branch_sim',
            name='Branch Simulation',
            priority=0.8,
            demand=cost
        )
    ...
    def generate_report(self) -> Dict:
        ...
        return {
            ...
            'consumer_efficiencies': {
                cid: self.get_average_efficiency(cid)
                for cid in self.efficiency_metrics.keys()
            }
        }
...
class AMOSEnergySystem:
    ...
    def _init_consumers(self):
        ...
        # Brain consumers
        self.register_consumer('compute', EnergyConsumer(
            consumer_id='brain_cognition',
            name='Brain Cognition',
            priority=0.9
        ))
        
        self.register_consumer('compute', EnergyConsumer(
            consumer_id='branch_generation',
            name='Branch Generation',
            priority=0.8
        ))
        
        self.register_consumer('compute', EnergyConsumer(
            consumer_id='simulation',
            name='Simulation Engine',
            priority=0.7
        ))
        
        self.register_consumer('compute', EnergyConsumer(
            consumer_id='morph_execution',
            name='Morph Execution',
            priority=0.6
        ))
        
        self.register_consumer('memory', EnergyConsumer(
            consumer_id='memory_storage',
            name='Memory Storage',
            priority=0.8
        ))
        
        self.register_consumer('memory', EnergyConsumer(
            consumer_id='memory_retrieval',
    ...
    def check_energy_constraints(self) -> Dict[str, Any]:
        ...
        for name, pool in self.pools.items():
            results[name] = {
                ...
                'utilization': pool.consumed / pool.total_capacity if pool.total_capacity > 0 else 0
            }
...
def demo_energy_system():
    ...
    for pool_name, pool_data in status['pools'].items():
        print(f"  {pool_name}: {pool_data['current']:.0f}/{pool_data['total']:.0f} "
    ...
    for pool_name, checks in constraints.items():
        ...
        print(f"  {status} {pool_name}: conservation={checks['conservation_satisfied']}, "
    ...
    for consumer_id, pool_name, energy_used, output in consumption:
        ...
        print(f"  {consumer_id}: {energy_used:.1f} energy → {output:.1f} output "
              f"(efficiency: {eff:.1%})")
