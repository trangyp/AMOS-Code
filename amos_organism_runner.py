...
class AMOSOrganismRunner:
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            if self.config.enable_layer_7:
                ...
                self.layer_7_validator = AxiomValidator(
                    strict=self.config.axiom_validation
                )
    ...
    async def calibrate(self) -> ValidationReport:
        ...
        if self.layer_7_validator:
            ...
            axiom_check = self.layer_7_validator.check_axiom_1_substrate(
                test_state
            )
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="AMOS Organism Runner"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--health-check", type=float, default=30.0,
        help="Health check interval (seconds)"
    )
    parser.add_argument(
        "--no-axiom-validation", action="store_true",
        help="Disable axiom validation"
    )
