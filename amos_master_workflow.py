...
class AMOSMasterWorkflow:
    ...
    def _run_phase4_report(self) -> WorkflowPhase:
        ...
        try:
            ...
            phase.results = {
                ...
                "phases_completed": len([p for p in self.current_run.phases if p.status == "completed"]),
            }
    ...
    def _generate_project_name(self, goal: str) -> str:
        ...
        key_words = [w for w in words if len(w) > 3 and w not in ["want", "build", "create", "make", "with", "using", "that", "this", "have", "need"]]
...
"""
    ...
    def _complete_workflow(self, status: str) -> WorkflowRun:
        ...
        print(f"   Completed: {len([p for p in self.current_run.phases if p.status == 'completed'])}")
