...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="AMOS Organism CLI", prog="amos-organism")
    ...
    # Start command
    start_parser = subparsers.add_parser("start", help="Start the organism")
    start_parser.add_argument("--name", type=str, help="Organism name")
    ...
    start_parser.add_argument(
        "--duration", type=float, default=None, help="Run for N seconds (default: indefinite)"
    )
    start_parser.add_argument(
        "--health-interval", type=float, default=30.0, help="Health check interval in seconds"
    )
    start_parser.set_defaults(func=cmd_start)

    # Stop command
    stop_parser = subparsers.add_parser("stop", help="Stop the organism")
    stop_parser.set_defaults(func=cmd_stop)

    # Restart command
    restart_parser = subparsers.add_parser("restart", help="Restart the organism")
    restart_parser.add_argument("--name", type=str, help="Organism name")
    restart_parser.add_argument(
        "--mode", default="development", choices=["development", "testing", "production"]
    )
    ...
    # Demo command
    demo_parser = subparsers.add_parser("demo", help="Run organism demo")
