...
class AMOSNATSClient:
    ...
    async def request(
        ...
        message = NATSMessage.create(topic=subject, payload=payload, source=self.repo_name)
    ...
    async def reply(
        self, subject: str, handler: Callable[[dict[str, Any]], dict[str, Any]]
    ) -> Subscription:
        ...
        async def message_handler(msg):
            try:
                ...
                response_payload = await asyncio.get_event_loop().run_in_executor(
                    None, lambda: handler(request.payload)
                )
    ...
    async def subscribe(
        self, subject: str, handler: Callable[[dict[str, Any]], None], queue: Optional[str] = None
    ) -> Subscription:
        ...
        async def message_handler(msg):
            try:
                ...
                await asyncio.get_event_loop().run_in_executor(
                    None, lambda: handler(message.payload)
                )
            except Exception as e:
    ...
    async def js_publish(self, subject: str, payload: dict[str, Any]) -> None:
    ...
    async def js_subscribe(
        self, subject: str, handler: Callable[[dict[str, Any]], None], durable: str, stream: str
    ) -> Subscription:
        ...
        sub = await self._js.subscribe(subject, durable=durable, stream=stream, cb=message_handler)
    ...
    async def scatter_gather(
        ...
        message = NATSMessage.create(
            topic=subject_pattern, payload=payload, source=self.repo_name, reply_to=inbox
        )
...
# === Singleton Instance ===_nats_client: Optional[AMOSNATSClient] = None
...
def init_nats_client(
    server_url: str, repo_name: str, jetstream_enabled: bool = True
) -> AMOSNATSClient:
