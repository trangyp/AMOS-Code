    ...
    def _compute_error(self, predicted: Dict, actual: Dict) -> float:
        ...
        for key in ['success', 'cost', 'risk']:
            if key in predicted and key in actual:
                p_val = float(predicted[key]) if isinstance(predicted[key], (int, float, bool)) else 0.5
...
class BranchEfficiencyAnalyzer:
    ...
    def record_branch_generation(self, n_branches: int, cost: float, 
                                  selected_branch_rank: int, outcome_success: bool):
...
class ParameterAdapter:
    ...
    def compute_update_signal(self, param_name: str, 
    ...
    def adapt(self, feedback: Dict[str, float]):
        ...
        for param_name, performance_delta in feedback.items():
            if param_name in self.current_params:
                ...
                # Keep within bounds
                self.current_params[param_name] = max(0.0, min(1.0, 
...
class AMOSMetaCognition:
    ...
    def reflect(self, cycle_data: Dict) -> ReflectionResult:
        ...
        if cycle_data.get('failure'):
            ...
            if patterns:
                ...
                insights.append(f"Detected repeating failure pattern: {top_pattern['occurrences']} times")
    ...
    def suggest_improvements(self) -> List[str]:
        ...
        patterns = self.failure_detector.detect_repeating_patterns()
        if patterns:
            suggestions.append(f"Address top failure pattern ({patterns[0]['occurrences']} occurrences)")
...
def demo_meta_cognition():
    ...
    cycles = [
        ...
        {
            'branch_stats': {'n_branches': 5, 'cost': 80, 'selected_rank': 2, 'outcome_success': True},
            'confidence': 0.7,
            'outcome': True
        },
        {
        ...
        {
            'branch_stats': {'n_branches': 3, 'cost': 50, 'selected_rank': 1, 'outcome_success': True},
            'confidence': 0.9,
            'outcome': True
        }
    ]
...
class ReflectionEngine(AMOSMetaCognition):
    ...
    def reflect(self, observation: Dict) -> Dict:
        ...
        result = super().reflect({
            'prediction_error': abs(observation.get('prediction', 0.5) - observation.get('actual', 0.5)),
        ...
        return {
            'accuracy': 1.0 - abs(observation.get('prediction', 0.5) - observation.get('actual', 0.5)),
