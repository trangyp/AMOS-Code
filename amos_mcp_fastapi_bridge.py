...
class RateLimiter:
    ...
    def is_allowed(self, api_key: str) -> tuple[bool, dict[str, Any]]:
        ...
        if entry.blocked_until and now < entry.blocked_until:
            return False, {"error": "Rate limit exceeded", "retry_after": int(entry.blocked_until - now)}
        ...
        entry.requests += 1
        self._limits[api_key] = entry
        return True, {"limit": self.requests_per_minute, "remaining": self.requests_per_minute - entry.requests}
...
@app.get("/")
async def root():
    return {"name": "AMOS MCP API", "version": "1.0.0", "docs": "/docs", "health": "/health", "tools": "/mcp/tools"}
...
async def list_tools(api_key: str = Depends(verify_api_key)):
    tools = [
        ToolInfo(name="brain_think", description="Process a thought", subsystem="brain",
                parameters={"thought": "string", "thought_type": "string (optional)"}),
        ToolInfo(name="brain_plan", description="Create a plan", subsystem="brain",
                parameters={"goal": "string", "horizon": "string (optional)"}),
        ToolInfo(name="brain_remember", description="Store in memory", subsystem="brain",
                parameters={"key": "string", "value": "any"}),
        ToolInfo(name="senses_scan", description="Scan environment", subsystem="senses",
                parameters={"path": "string (optional)", "depth": "integer (optional)"}),
        ToolInfo(name="senses_gather", description="Gather context", subsystem="senses",
                parameters={"topic": "string", "sources": "list (optional)", "max_items": "integer (optional)"}),
        ToolInfo(name="muscle_execute", description="Execute command", subsystem="muscle",
                parameters={"command": "string", "timeout_seconds": "integer (optional)"}),
        ToolInfo(name="muscle_code", description="Run code", subsystem="muscle",
                parameters={"code": "string", "language": "string (optional)"}),
        ToolInfo(name="immune_validate", description="Validate action", subsystem="immune",
...
async def call_tool(request: ToolCallRequest, api_key: str = Depends(verify_api_key)):
    ...
    if not bridge._initialized:
        raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="Cognitive bridge not initialized")
    ...
    try:
        ...
        return ToolCallResponse(
            request_id=request_id, tool=request.tool, success=response.success,
            result=response.result, execution_time_ms=execution_time,
        ...
        execution_time = (time.time() - start_time) * 1000
        return ToolCallResponse(
            request_id=request_id, tool=request.tool, success=False,
            result={"error": str(e)}, execution_time_ms=execution_time,
            health_score=bridge.get_stats().get("health_score", 0.0),
            timestamp=datetime.now(timezone.utc).isoformat()
        )
...
async def call_tool_stream(request: ToolCallRequest, api_key: str = Depends(verify_api_key)):
    ...
    async def event_generator() -> AsyncGenerator[str, None]:
        ...
        try:
            ...
            execution_time = (time.time() - start_time) * 1000
            result = {
                "event": "complete", "request_id": request_id, "tool": request.tool,
                "success": response.success, "result": response.result,
                "execution_time_ms": execution_time, "timestamp": datetime.now(timezone.utc).isoformat()
            }
            yield f"data: {result}\n\n"
        except Exception as e:
            execution_time = (time.time() - start_time) * 1000
            error = {
                "event": "error", "request_id": request_id, "tool": request.tool,
