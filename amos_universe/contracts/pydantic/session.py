...
class Workspace(BaseAMOSModel):
    ...
    quota: dict[str, int] = Field(
        default_factory=dict,
        description="Usage quotas (users, api_keys, etc)"
    )
    created_at: datetime = Field(
        default_factory=lambda: datetime.now(timezone.utc),
...
class SessionContext(BaseAMOSModel):
    ...
    workspace_id: Optional[str] = Field(None, description="Active workspace ID")
    ...
    client_info: dict[str, Any] = Field(
        default_factory=dict,
        description="Client information (user_agent, ip, etc)"
    )
...
class UserSession(BaseAMOSModel):
    ...
    workspace: Optional[Workspace] = Field(None, description="Active workspace")
