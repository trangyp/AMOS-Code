...
class ScanIssue(BaseAMOSModel):
    ...
    confidence: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Confidence in this issue (0-1)"
    )
...
class RepoScanRequest(BaseAMOSModel):
    ...
    scan_types: list[str] = Field(
        default_factory=lambda: ["style", "typing", "security"],
...
class RepoScanResult(BaseAMOSModel):
    ...
    status: str = Field(
        ...,
        description="Scan status: pending, running, completed, failed"
    )
...
class RepoFixResult(BaseAMOSModel):
    ...
    status: str = Field(
        ...,
        description="Fix status: preview, applied, partial, failed"
    )
    changes: list[FileChange] = Field(
        default_factory=list,
        description="File changes to be applied or that were applied"
    )
    applied: bool = Field(
        ...,
        description="Whether changes were actually applied"
    )
    summary: dict[str, Any] = Field(
        default_factory=dict,
        description="Fix summary statistics"
    )
    started_at: datetime = Field(
        default_factory=lambda: datetime.now(timezone.utc),
