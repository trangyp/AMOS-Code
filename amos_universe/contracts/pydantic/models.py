...
class ModelCapabilities(BaseAMOSModel):
    ...
    context_window: int = Field(..., ge=1024, description="Maximum context window in tokens")
    max_output_tokens: int = Field(..., ge=1, description="Maximum output tokens per request")
    ...
    supports_json_mode: bool = Field(
        default=False, description="Whether the model supports constrained JSON output"
    )
    quantization: str = Field(None, description="Quantization level (e.g., 'Q4_K_M', 'Q8_0')")
...
class ModelInfo(BaseAMOSModel):
    ...
    size_gb: float = Field(None, ge=0.0, description="Model size in gigabytes")
    tags: list[str] = Field(
        default_factory=list, description="Model tags (e.g., ['coding', 'chat'])"
    )
    is_local: bool = Field(default=True, description="Whether the model runs locally")
    is_loaded: bool = Field(
        default=False, description="Whether the model is currently loaded in memory"
    )
    loaded_at: datetime = Field(None, description="When the model was loaded")
...
class ModelRequest(BaseAMOSModel):
    ...
    system_prompt: str = Field(None, description="System prompt/instructions")
    ...
    response_format: str = Field(None, description="Response format (e.g., 'json')")
...
class ModelResponse(BaseAMOSModel):
    ...
    finish_reason: str = Field(None, description="Why generation stopped")
    tool_calls: list[dict[str, Any]] = Field(
        default_factory=list, description="Tool calls requested"
    )
    generation_time_ms: int = Field(None, ge=0, description="Generation time in milliseconds")
