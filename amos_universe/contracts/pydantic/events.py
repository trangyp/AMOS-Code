...
class EventMetadata(BaseAMOSModel):
    ...
    trace_id: str = Field(default=None, description="Distributed trace ID for request correlation")

    tenant_id: str = Field(default=None, description="Multi-tenant workspace ID (if applicable)")
...
class BaseEvent(BaseAMOSModel):
    ...
    metadata: EventMetadata = Field(..., description="Event metadata (tracing, routing, audit)")
