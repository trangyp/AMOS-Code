...
class AMOSWorkflow:
    ...
    def code_review(self, filepath: str) -> dict[str, Any]:
        ...
        review_context = {
            ...
            "has_types": "def " in code and ("-> " in code or ": " in code.split("def")[1].split(":")[0] if "def " in code else False)
        }
...
"""
        ...
        if hasattr(amos, 'reasoning') and amos.reasoning:
            ...
            print(f"  Primary perspective: {rule_of_two.get('perspectives', [{}])[0].get('name', 'N/A')}")
            print(f"  Alternative perspective: {rule_of_two.get('perspectives', [{}, {}])[1].get('name', 'N/A')}")
...
def main():
    parser = argparse.ArgumentParser(
        description="AMOS Brain Workflow Examples"
    )
    parser.add_argument(
        'task',
        nargs='?',
    ...
    parser.add_argument(
        '--file',
        default='clawspring/agent.py',
        help='File to review (for review task)'
    )
    parser.add_argument(
        '--decision',
    ...
    try:
        if args.task == 'review':
            ...
            result = workflow.architecture_decision(
                args.decision,
                ['async/await', 'threading', 'sync with pools']
            )
            if 'error' in result:
                print(f"\nError: {result['error']}")
                return 1
            ...
            result = workflow.refactoring_plan(
                'clawspring/tools.py',
                ['Line too long', 'Unused imports', 'Complex functions']
            )
            if 'error' in result:
                print(f"\nError: {result['error']}")
                return 1
            ...
            workflow.architecture_decision(
                "Should we use async or sync I/O?",
                ['async/await', 'threading', 'sync with pools']
            )
            
            # Task 3: Refactoring Plan
            print("\n\n>>> TASK 3: REFACTORING PLAN <<<")
            workflow.refactoring_plan(
                'clawspring/tools.py',
                ['Line too long', 'Unused imports']
            )
