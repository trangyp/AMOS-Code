...
class TemporalPath:
    ...
    def compute_metrics(self, goal_state: Dict[str, Any]):
        ...
        self.risk = sum(
            e.payload.get('risk', 0.0) for e in self.events
        ...
        self.total_reward = sum(
            e.payload.get('reward', 0.0) for e in self.events
        )
...
class TimeEngine:
    ...
    def project_future(self, 
                       ...
                       n_paths: int = 3) -> List[TemporalPath]:
        """
    ...
    def _project_single_path(self, action: Dict, path_id: str) -> TemporalPath:
        ...
        projected_state = self._apply_action(self.present.state_data if self.present else {}, action)
    ...
    def create_recovery_point(self, label: str = "") -> datetime:
        ...
        event = Event(
            ...
            payload={
                'label': label,
                'state': self.present.state_data if self.present else {}
            }
        )
        self.event_store.append(event)
...
def demo_time_engine():
    ...
    for path in paths:
        print(f"    - {path.path_id}: {len(path.events)} events, "
