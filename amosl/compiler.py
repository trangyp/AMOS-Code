...
from .ast_nodes import (
    Program, OntologyDecl, StateDecl, DynamicsDecl,
    EntityDecl, QuantumEntityDecl, BioEntityDecl, HybridEntityDecl,
    StateVar, QuantumState, BioState, HybridState,
    Transition, ActionDecl, Evolution,
)
from .ir import (
    CIR, QIR, BIR, HIR,
    CIRBlock, CIROp,
    QIRRegister, QIRGate, QIRMeasure,
    BIRSpecies, BIRSequence, BIRReaction,
    HIRBridge, HIRSchedule, HIRObservation,
)
...
class IRGenerator:
    ...
    def process_quantum_entity(self, entity: QuantumEntityDecl) -> None:
        ...
        if entity.circuit:
            for gate_name in entity.circuit.gates:
                self.qir.gates.append(QIRGate(
                    name=gate_name,
                    targets=list(range(entity.qubits))
                ))
    ...
    def process_bio_entity(self, entity: BioEntityDecl) -> None:
        ...
        species = BIRSpecies(
            name=entity.name,
            type_name=entity.protein or "gene"
        )
        self.bir.species.append(species)
        
        # Add sequence if DNA/RNA defined
        if entity.dna:
            self.bir.sequences.append(BIRSequence(
                name=f"{entity.name}_dna",
                seq_type="dna",
                sequence=entity.dna
            ))
    ...
    def process_state(self, state: StateDecl) -> None:
        ...
        for var in state.classical:
            self.cir.blocks.append(CIRBlock(
        ...
        for var in state.quantum:
            self.qir.registers.append(QIRRegister(
                name=var.name,
                size=1,
        ...
        for var in state.biological:
            self.bir.species.append(BIRSpecies(
                name=var.name,
                initial_conc=var.concentration.text if var.concentration else 0.0
            ))
        
        # Hybrid state -> HIR bridges
        for var in state.hybrid:
            self.hir.bridges.append(HIRBridge(
                name=var.name,
                source_var=var.source,
    ...
    def process_dynamics(self, dynamics: DynamicsDecl) -> None:
        ...
        for evo in dynamics.evolutions:
            if evo.rule == "mutate":
                ...
                self.bir.reactions.append(BIRReaction(
                    reactants=[evo.target],
                    products=[f"{evo.target}_mutated"]
                ))
            else:
                # General evolution schedule
                self.hir.schedules.append(HIRSchedule(
                    name=f"evolve_{evo.target}",
                    order=[evo.rule]
                ))
