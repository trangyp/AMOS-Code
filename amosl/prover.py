...
class TheoremProver:
    ...
    def prove_bridge_legal(self, bridge_id: str, source: str, target: str,
                          type_compat: bool = True,
                          unit_compat: bool = True,
                          time_compat: bool = True) -> Proof:
        """Prove Legal(B_ij) = TypeCompat · UnitCompat · TimeCompat."""
        steps = [f"Init: Legal(B_{{{source}->{target}}})",
        ...
        if all_legal:
            return Proof(
                statement=f"Legal(B_{{{bridge_id}}}) = 1",
                status=ProofStatus.PROVEN,
                steps=steps
            )
        else:
    ...
    def prove_type_derivation(self, expr: str, gamma: Dict[str, str],
                             expected_type: str) -> Proof:
        """Prove Γ ⊢ e:τ."""
        steps = [f"Init: Γ ⊢ {expr}:{expected_type}",
                f"Context Γ = {gamma}"]
    ...
    def _tactic_split(self, goal: str, context: Dict) -> Proof:
        ...
        return Proof(
            statement=f"Split: {goal}",
            status=ProofStatus.UNPROVEN,
            steps=["Decompose conjunction"]
        )
    ...
    def get_statistics(self) -> Dict[str, int]:
        ...
        for status in ProofStatus:
            stats[status.name.lower()] = sum(
                1 for p in self.proof_history if p.status == status
            )
        return stats
