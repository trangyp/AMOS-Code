...
class AxiomChecker:
    ...
    def check_all_axioms(self, system_state: Dict[str, Any]) -> Dict[AxiomID, AxiomCheckResult]:
        ...
        results[AxiomID.OBSERVATION_NON_NEUTRALITY] = self.check_observation_non_neutrality(system_state)
    ...
    def check_semantic_primacy(self, state: Dict[str, Any]) -> AxiomCheckResult:
        ...
        return AxiomCheckResult(
            ...
            satisfied=satisfied,
            evidence={'has_encoder': has_encoder, 'has_ontology': has_ontology, 'has_types': has_types},
            message="Semantic primacy satisfied: Enc(O,T,C) → S exists" if satisfied else "Missing encoding mapping"
        )
    ...
    def check_typed_existence(self, state: Dict[str, Any]) -> AxiomCheckResult:
        ...
        return AxiomCheckResult(
            ...
            message=f"All {len(entities)} entities typed" if all_typed else f"Untyped entities: {untyped}"
        )
    ...
    def check_stratified_state(self, state: Dict[str, Any]) -> AxiomCheckResult:
        ...
        return AxiomCheckResult(
            ...
            message=f"Stratified state: {len(present)}/6 dimensions present" + (" ✓" if all_present else "")
        )
    ...
    def check_invariant_gated_commit(self, state: Dict[str, Any]) -> AxiomCheckResult:
        ...
        return AxiomCheckResult(
            ...
            evidence={'constraints_count': len(constraints), 'has_commit_logic': commit_logic is not None},
            message=f"Invariant-gated commit: {len(constraints)} constraints" if has_gated_commit else "No commit gating"
        )
    ...
    def check_observation_non_neutrality(self, state: Dict[str, Any]) -> AxiomCheckResult:
        ...
        return AxiomCheckResult(
            ...
            satisfied=satisfied,
            evidence={'has_observation': has_observation, 'has_perturbation': has_perturbation, '[M,F]≠0': dynamics_affects_obs},
            message="Observation non-neutral: [M,F]≠0" if satisfied else "Observation may be neutral"
        )
    ...
    def check_bridge_explicitness(self, state: Dict[str, Any]) -> AxiomCheckResult:
        ...
        return AxiomCheckResult(
            ...
            evidence={'transfers': len(transfers), 'bridges': len(bridges), 'unbridged': len(unbridged)},
            message=f"All {len(transfers)} transfers have bridges" if all_have_bridges else f"Missing bridges for: {unbridged}"
        )
    ...
    def check_admissible_adaptation(self, state: Dict[str, Any]) -> AxiomCheckResult:
        ...
        return AxiomCheckResult(
            ...
            message=f"All {len(adaptations)} adaptations preserve validity" if all_valid else f"Invalid adaptations: {len(invalid_adaptations)}"
        )
    ...
    def check_ledger_completeness(self, state: Dict[str, Any]) -> AxiomCheckResult:
        ...
        return AxiomCheckResult(
            ...
            message=f"Ledger complete: {len(ledger_entries)}/{len(transitions)} transitions recorded" if all_recorded else "Missing ledger entries"
        )
    ...
    def check_explainability(self, state: Dict[str, Any]) -> AxiomCheckResult:
        ...
        return AxiomCheckResult(
            ...
            message="Explainability: Explain(L) = Outcome defined" if satisfied else "Missing explanation mechanism"
        )
