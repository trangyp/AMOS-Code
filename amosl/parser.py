...
from .ast_nodes import (
    ...
    ConstraintDecl, EffectDecl, MeasureDecl, UncertaintyDecl,
    VerifyDecl, AdaptDecl, RealizeDecl,
    ...
    Field, Circuit, Reaction, Bridge, Mapping, Signal, Expr,
    Location, Substrate,
)
...
class Lexer:
    ...
    KEYWORDS = {
        'ontology', 'state', 'dynamics', 'constraint', 'effect',
        'measure', 'uncertainty', 'verify', 'adapt', 'realize',
        'classical', 'quantum', 'biological', 'hybrid',
        'entity', 'qubit', 'gene', 'protein', 'bridge',
        'action', 'transition', 'evolve', 'invariant',
        'true', 'false', 'unknown', 'probable',
    }
...
class Parser:
    ...
    def expect(self, type_: str) -> Token:
        if self.current().type != type_:
            raise ParseError(
                f"Expected {type_}, got {self.current().type} "
                    ...
                    while not self.match('}'):
                        if self.match('IDENT'):
                            ...
                            entity.fields.append(
                                Field(field_name, field_type, loc=self.location())
                            )
                        else:
                            self.advance()
                    self.expect('}')
        ...
        while not self.match('}'):
            if self.match('QUBIT'):
                ...
                if self.match('['):
                    ...
                    self.expect(']')
                entities.append(
                    QuantumEntityDecl(name, size, loc=self.location())
                )
            else:
                self.advance()
