...
Examples:
  ...
  amosl service start --port=8080
    ...
    parser.add_argument(
        "--version",
        action="version",
        version="AMOSL v4.0.0 - Field-Theoretic Programming System"
    )
    ...
    run_parser.add_argument("--substrate", choices=["c", "q", "b", "h", "hybrid"],
                           default="hybrid", help="Execution substrate")
    run_parser.add_argument("--verify", action="store_true",
                           help="Verify admissibility before execution")
    ...
    verify_parser.add_argument("--axioms", choices=["all", "core"],
                              default="all", help="Axioms to check")
    ...
    evolve_parser.add_argument("--steps", type=int, default=50,
                              help="Number of evolution steps")
    evolve_parser.add_argument("--dt", type=float, default=0.1,
                            help="Time step size")
    evolve_parser.add_argument("--action", action="store_true",
                              help="Compute action functional")
    
    # prove command
    prove_parser = subparsers.add_parser("prove", help="Run theorem prover")
    prove_parser.add_argument("theorem", choices=["grand_admissibility", "state_validity",
                                                  "bridge_legality", "type_derivation"],
                             help="Theorem to prove")
    prove_parser.add_argument("--state", help="State file for verification")
    
    # benchmark command
    bench_parser = subparsers.add_parser("benchmark", help="Run performance benchmarks")
    bench_parser.add_argument("--suite", choices=["quick", "full"],
                             default="quick", help="Benchmark suite")
    ...
    ledger_parser.add_argument("action", choices=["show", "explain", "verify"],
                              help="Ledger action")
    ...
    service_parser.add_argument("action", choices=["start", "stop", "status"],
                               help="Service action")
    service_parser.add_argument("--port", type=int, default=8080,
                               help="Service port")
...
def cmd_run(args) -> int:
    ...
    try:
        ...
        print(f"Error: Invalid JSON in program file")
        return 1
...
def cmd_verify(args) -> int:
    ...
    if args.program:
        try:
            ...
            print(f"Warning: State file not found, using empty state")
...
def cmd_evolve(args) -> int:
    ...
    print(f"AMOSL: Running field evolution")
...
def cmd_service(args) -> int:
    ...
    if args.action == "start":
        ...
        print("  Endpoints:")
        print(f"    POST /run     - Execute program")
        print(f"    POST /verify  - Verify admissibility")
