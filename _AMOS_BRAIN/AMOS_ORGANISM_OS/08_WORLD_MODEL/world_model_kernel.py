...
class WorldModelKernel:
    ...
    def add_relationship(self, from_id: str, to_id: str, relation: SpatialRelation) -> bool:
    ...
    def _infer_facts(self, context: Context):
        ...
        recent_count = sum(
            ...
            and (now - datetime.fromisoformat(self.objects[obj_id].last_updated))
            < timedelta(minutes=1)
        )
        if recent_count > 0:
    ...
    def get_object_graph(self) -> dict[str, Any]:
        ...
        return {
            ...
            "edges": [
                {"source": obj_id, "target": target_id, "relation": rel_type}
                for obj_id, obj in self.objects.items()
