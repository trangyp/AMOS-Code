...
class MuscleKernel:
    ...
    def invoke_tool(self, tool_id: str, parameters: Dict[str, Any], source: str = "unknown") -> Action:
    ...
    def get_state(self) -> Dict[str, Any]:
        ...
        return {
            "active_actions": len([a for a in self.actions.values() if a.status == ActionStatus.EXECUTING]),
            "pending_actions": len([a for a in self.actions.values() if a.status == ActionStatus.PENDING]),
...
if __name__ == "__main__":
    ...
    action1 = muscle.execute_action(
        ...
        {"path": str(root / "00_ROOT" / "root_manifest.json")},
        "TEST"
    )
