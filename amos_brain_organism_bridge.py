...
from amos_brain.cookbook import ArchitectureDecision, ProblemDiagnosis
...
class BrainOrganismBridge:
    ...
    def _classify_action(self, task: str, analysis: dict) -> str:
        ...
        # Check task patterns
        if any(
            kw in task_lower for kw in ["refactor", "implement", "write code", "create function"]
        ):
...
def demo_bridge():
    ...
    print("-" * 70)
    result = bridge.analyze_and_execute(
        "Database connection pool exhaustion", context={"symptoms": ["500 errors", "high latency"]}
    )
    print(f"\nStatus: {result.status}")
    print(f"Action: {result.organism_action}")
    print(f"Output:\n{result.output}")
    ...
    print("-" * 70)
    result = bridge.analyze_and_execute("Refactor the authentication module to use JWT tokens")
    print(f"\nStatus: {result.status}")
    print(f"Action: {result.organism_action}")
    print(f"Output:\n{result.output}")
