...
class KnowledgeEngine:
    ...
    def discover_engines(self) -> dict[str, int]:
        ...
        if tech_path.exists():
            for f in tech_path.glob("*.json"):
                ...
                self.engines[name] = EngineInfo(
                    name=name, domain="tech", path=str(f), size=f.stat().st_size
                )
        ...
        if cognitive_path.exists():
            for f in cognitive_path.glob("*.json"):
                name = f.stem
                self.engines[name] = EngineInfo(
                    name=name, domain="cognitive", path=str(f), size=f.stat().st_size
                )
        ...
        if unipower_path.exists():
            for f in unipower_path.glob("*.json"):
                name = f.stem
                self.engines[name] = EngineInfo(
                    name=name, domain="unipower", path=str(f), size=f.stat().st_size
                )
        ...
        if domains_path.exists():
            for f in domains_path.glob("*.json"):
                name = f.stem
                self.engines[name] = EngineInfo(
                    name=name, domain="domain", path=str(f), size=f.stat().st_size
                )
    ...
    def query(self, domain: str, question: str) -> dict[str, Any]:
        ...
        relevant = [
            e for e in self.engines.values() if e.domain == domain or domain in e.name.lower()
        ]
