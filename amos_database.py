...
class AMOSDatabase:
    ...
    async def store_health(self, overall_status: str, checks: List[Dict], 
    ...
    def _store_health_sync(self, overall_status: str, checks: List[Dict], 
                          uptime_seconds: float):
        with sqlite3.connect(self.db_path) as conn:
            conn.execute('''
    ...
    async def store_alert(self, alert_id: str, rule_name: str, severity: str,
                          status: str, message: str, value: float, 
                          threshold: float):
        """Store an alert."""
        async with self._lock:
            loop = asyncio.get_event_loop()
            return await loop.run_in_executor(
                None, self._store_alert_sync, alert_id, rule_name, severity,
                status, message, value, threshold
            )
    
    def _store_alert_sync(self, alert_id: str, rule_name: str, severity: str,
                          status: str, message: str, value: float, threshold: float):
        with sqlite3.connect(self.db_path) as conn:
            conn.execute('''
                INSERT OR REPLACE INTO alerts 
                (alert_id, rule_name, severity, status, message, value, threshold, timestamp)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            ''', (alert_id, rule_name, severity, status, message, value, 
                  threshold, datetime.utcnow().isoformat()))
            conn.commit()
    
    async def get_query_history(self, limit: int = 100, 
...
if __name__ == '__main__':
    ...
    async def test():
        ...
        # Test query logging
        await db.log_query(QueryRecord(
        ...
        # Test metrics
        await db.store_metric(MetricRecord(
