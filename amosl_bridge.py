...
from amosl_ledger import StateLedger, TransactionLog, EntryType
...
class BridgeExecutor:
    ...
    def execute_cross_substrate(
        ...
        op_id = f"BRIDGE-{datetime.utcnow().strftime('%Y%m%d%H%M%S')}-{hash(str(payload)) % 10000:04d}"
    ...
    def get_bridge_statistics(self) -> dict[str, Any]:
        ...
        bridge_ops = [
            e for e in self.ledger._entries
            if e.entry_type == EntryType.BRIDGE_OPERATION
        ]
        
        success_count = sum(
            1 for op in bridge_ops
