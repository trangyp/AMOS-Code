...
class IdentityContinuity:
    ...
    def add_successor(
        self, successor_id: str, transform: IdentityTransform, justification: str
    ) -> bool:
...
class EquivalenceClaim:
    ...
    # Wrapper detection
    mediated_by: str = None  # Adapter/shim name
    wrapper_truthful: bool = True

    def check_contextual_validity(self, regime: str, mode: str, trust_domain: str) -> bool:
        """I_contextual_equivalence = 1 iff equivalence is regime-tagged."""
        return (
            regime in self.valid_regimes
            and mode in self.valid_modes
            and trust_domain in self.valid_trust_domains
        )
...
class DisagreementRegistry:
    ...
    def resolve(self, disagreement_id: str, resolution: str, arbiter: str) -> bool:
    ...
    def approve(self, modification_id: str, authority: str) -> bool:
        ...
        if modification_id in self.modifications:
            ...
            mod.audit_trail.append(
                {"event": "approved", "timestamp": datetime.now().isoformat(), "by": authority}
            )
            return True
        return False
...
class MetaGovernance:
    ...
    emergency_authority: str = None
    emergency_decay_timer: datetime = None
