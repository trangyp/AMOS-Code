...
class AgentReasoningEngine:
    ...
    async def execute_react(
        ...
        # Add initial thought
        self._add_thought(chain, ThoughtType.REASONING,
                         f"Starting task: {chain.task}", 0)
        ...
        while iteration < max_iterations:
            ...
            if "Action:" in response:
                ...
                self._add_thought(chain, ThoughtType.REASONING,
                                thought_content, iteration)

                # Execute action
                action_result = await self._execute_action(action_content)

                # Add action and observation
                self._add_thought(chain, ThoughtType.ACTION,
                                action_content, iteration)
                self._add_thought(chain, ThoughtType.OBSERVATION,
                                str(action_result), iteration)
                ...
                self._add_thought(chain, ThoughtType.CONCLUSION,
                                answer, iteration)
    ...
    async def execute_chain_of_thought(
        ...
        self._add_thought(chain, ThoughtType.PLAN,
                         "Breaking down problem into steps", 0)
...
Execute each step and provide the final result."""
    ...
    def _add_thought(self, chain: ReasoningChain,
