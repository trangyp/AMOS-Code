...
class KnowledgeIntegratedReasoning:
        ...
        for engine_key, knowledge in active.items():
            ...
            if score > 0.1:  # Minimum relevance threshold
                ...
                if hasattr(knowledge, 'content') and isinstance(knowledge.content, dict):
                    ...
                    elif 'meta' in knowledge.content and isinstance(knowledge.content['meta'], dict):
                        excerpt = str(knowledge.content['meta'])[:200]
                    else:
                        excerpt = str(list(knowledge.content.values())[0])[:200] if knowledge.content else ""
                
                matches.append(KnowledgeMatch(
                    engine_key=engine_key,
                    category=knowledge.category if hasattr(knowledge, 'category') else 'unknown',
    ...
    def reason_with_knowledge(self, problem: str, use_rule_of_2: bool = True, use_rule_of_4: bool = True) -> dict[str, Any]:
    ...
    def _base_reasoning(self, problem: str, use_rule_of_2: bool, use_rule_of_4: bool) -> dict[str, Any]:
    ...
    def _augment_with_knowledge(self, base_analysis: dict, relevant: list[KnowledgeMatch]) -> dict[str, Any]:
    ...
    def _generate_conclusion(self, problem: str, base: dict, augmented: dict, relevant: list) -> dict[str, Any]:
        ...
        if len(relevant) >= 3:
            ...
            conclusion["recommendation"] = "High confidence: Multiple knowledge sources aligned. Proceed with integrated approach."
        elif len(relevant) >= 1:
            conclusion["confidence"] = 0.65
            conclusion["recommendation"] = "Moderate confidence: Limited knowledge sources available. Consider activating more engines."
        else:
            conclusion["confidence"] = 0.45
            conclusion["recommendation"] = "Low confidence: No relevant knowledge activated. Activate appropriate engines first."
...
Examples:
  ...
  python amos_reasoning_with_knowledge.py demo
    ...
    parser.add_argument("command", nargs="?", default="demo",
                       choices=["reason", "query", "augment", "demo"])
