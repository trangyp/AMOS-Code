...
from amosl.field import FieldEvolution, FieldState, LagrangianTerms
from amosl.geometry import InformationGeometry, BeliefManifold, BeliefState
from amosl.modal import ModalLogic, StratifiedTruth, TruthValue, AdmissibilityLogic
...
def demo_information_geometry():
    ...
    # Belief states
    prior = BeliefState(
        distribution={'state_a': 0.6, 'state_b': 0.4},
        timestamp=0.0
    )
...
def demo_field_theoretic():
    ...
    print(f"  Biological concentrations: {len(initial.biological.get('concentrations', []))} dimensions")
