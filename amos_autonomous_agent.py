...
class AMOSAutonomousAgent:
    ...
    def report_results(self, result: ExecutionResult) -> None:
        ...
        for i, step in enumerate(result.plan.steps, 1):
            icon = "✅" if step.status == ExecutionStatus.COMPLETED else "❌" if step.status == ExecutionStatus.FAILED else "⏭️"
    ...
    def _generate_project_name(self, goal: str) -> str:
        ...
        words = [w for w in goal.split() if len(w) > 3 and w.lower() not in ["want", "need", "build", "create", "system", "tool"]]
        if words:
...
def main():
    ...
    parser.add_argument(
        "goal",
        nargs="?",
        help="Goal to accomplish (e.g., 'Build a medical decision system')"
    )
    parser.add_argument(
        "--interactive",
        action="store_true",
