...
class PersistentState:
    ...
    cumulative_metrics: Dict[str, float] = field(default_factory=lambda: {
        'total_cycles': 0,
...
class AMOSv4:
    ...
    def __init__(self, name: str = "AMOS_v4"):
        ...
        if self.persistent_state is None:
            ...
            print(f"🔄 AMOS v4 resumed from previous session")
            print(f"   Previous cycles: {self.persistent_state.cumulative_metrics['total_cycles']}")
            print(f"   Total value produced: {self.persistent_state.cumulative_metrics['total_value_produced']:.2f}")
    ...
    def cycle(self, world_signals: Optional[Dict] = None) -> Dict:
        ...
        self.persistent_state = self.persistence.sync(
            ...
            result, 
            self.persistent_state.identity
        )
...
def demo_v4():
    ...
    amos.goals.add_goal(Goal(
        id="g1", name="memory_optimization",
        description="Optimize memory system efficiency",
        priority=0.9, expected_value=100, time_horizon=30,
        resource_cost={'time': 20, 'compute': 50}
    ))
    amos.goals.add_goal(Goal(
        id="g2", name="economic_capability",
        description="Build economic decision engine",
        priority=0.8, expected_value=150, time_horizon=60,
        resource_cost={'time': 40, 'compute': 100}
    ))
    amos.goals.add_goal(Goal(
        id="g3", name="self_preservation",
        description="Enhance survival mechanisms",
        priority=1.0, expected_value=200, time_horizon=90,
