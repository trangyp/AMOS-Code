...
class AMOSScenarioDemo:
    ...
    def _allocate_portfolio(self, viable: List[Dict], scenario: Dict, day: int) -> Dict:
        ...
        print(f"  Compounding investment: {sum(1 for a in allocation.values() if 'COMPOUNDING' in a['reason'])} projects")
        ...
        return {
            ...
            'identity_drift': sum(a.get('drift_score', 0.05) for a in viable if a['id'] in allocation)
        }
    ...
    def _show_progress(self, decisions: List[Dict], day: int):
        ...
        if avg_drift > 0.15:
            print(f"  ⚠ WARNING: Identity drift elevated - consider recovery")
    ...
    def _show_final_summary(self, decisions: List[Dict]):
        ...
        print(f"  ✓ Did NOT choose single path (survival vs compounding)")
        print(f"  ✓ Allocated portfolio across multiple opportunities")
        print(f"  ✓ Maintained identity (health preservation)")
        print(f"  ✓ Balanced immediate cash with future value")
        print(f"  ✓ Used uncertainty modeling in decisions")
        print(f"  ✓ Adaptive allocation based on constraints")
        
        print(f"\n[Comparison: v3 vs v4 Decision]")
        print(f"  v3 approach: Pick 'best' single option")
        print(f"    → Would choose: Client A (immediate cash only)")
        print(f"    → Risk: No compounding, skill stagnation")
        print(f"\n  v4 Production approach: Portfolio allocation")
        print(f"    → Allocated: Client A (survival) + Product (compounding) + Skills (optionality)")
        print(f"    → Result: Survives AND compounds")
