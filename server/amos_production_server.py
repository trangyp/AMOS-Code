...
def initialize_components():
    ...
    active = sum(
        [
            amos_unified is not None,
            memory_manager is not None,
            mcp_bridge is not None,
            repo_doctor is not None,
        ]
    )
...
async def spawn_agent(request: AgentSpawnRequest):
    ...
    try:
        agent = amos_unified.spawn_agent(
            role=request.role, paradigm=request.paradigm, name=request.name
        )
...
def main():
    ...
    uvicorn.run(
        "amos_production_server:app", host="0.0.0.0", port=8000, reload=False, log_level="info"
    )
