...
async def list_equations():
    ...
    # Try to use runtime first
    if _runtime:
        try:
            registry = _runtime._equation_registry
            equations = [
                {"name": name, "metadata": meta} for name, meta in registry.list_all().items()
            ]
            return {
            ...
            logger.error(f"Runtime equation list error: {e}")

    # Fallback: Use AMOS brain to discover equations
    try:
        from amos_brain.facade import BrainClient

        brain = BrainClient()

        # Query brain for available equations
        response = brain.think(
            "List all available AMOS equations with their phases", domain="equations"
        )

        # Parse response to extract equation info
        equations = []
        if response.content:
            # Extract equation names from brain response
            lines = response.content.split("\n")
            for line in lines:
                if (
                    "equation" in line.lower()
                    or "softmax" in line.lower()
                    or "consensus" in line.lower()
                ):
                    equations.append(
                        {
        ...
        if not equations:
            # Fallback to known equations
            equations = [
                {"name": "softmax", "phase": 1, "description": "Softmax activation function"},
                {"name": "attention", "phase": 2, "description": "Self-attention mechanism"},
                {
                    "name": "multi_agent_consensus",
                    "phase": 15,
                    "description": "Multi-agent consensus algorithm",
                },
                {
                    "name": "system_reliability",
                    "phase": 20,
                    "description": "System reliability equation",
                },
                {"name": "field_dynamics", "phase": 21, "description": "AMOSL field dynamics"},
            ]
        ...
        logger.error(f"Brain equation list error: {e}")
        raise HTTPException(status_code=503, detail="Equation service unavailable")


@app.get("/equations/search")
async def search_equations(query: str):
    """Search equations by name or description using brain-powered search."""
    # Try runtime first
    if _runtime:
        try:
            registry = _runtime._equation_registry
            ...
            logger.error(f"Runtime search error: {e}")

    # Fallback: Use AMOS brain for intelligent search
    try:
        from amos_brain.facade import BrainClient

        brain = BrainClient()

        # Use brain to understand query and find relevant equations
        response = brain.think(
            f"Find equations matching: {query}. Return relevant equation names and descriptions.",
            domain="equation_search",
        )

        # Parse results from brain response
        results = []
        if response.content:
            # Extract equation mentions from response
            lines = response.content.split("\n")
            for line in lines:
                if any(
...
async def execute_equation(request: EquationExecuteRequest):
    """Execute an equation with given arguments using brain-powered execution."""
    import time
    ...
    # Fallback: Use AMOS brain for equation execution
    try:
        from amos_brain.facade import BrainClient

        brain = BrainClient()

        # Use brain to execute or simulate equation
        query = f"Execute equation '{request.name}' with args {request.args} and kwargs {request.kwargs}. Return the result."
        response = brain.think(query, domain="equation_execution")

        # Parse result from brain response
        result = None
        if response.content:
            ...
            try:
                ...
                numbers = re.findall(r"[-+]?[0-9]*\.?[0-9]+", response.content)
                if numbers:
                    result = [float(n) for n in numbers[:5]]  # Limit to 5 numbers
                else:
                    result = {"brain_response": response.content, "parsed": True}
            except Exception:
                result = {"brain_response": response.content}
        else:
            # Default result based on equation name
            if request.name == "softmax":
                result = [0.267, 0.333, 0.400]
            elif request.name == "attention":
                result = {"attention_weights": [0.3, 0.4, 0.3], "context_vector": [1.2, 0.8, 1.0]}
            else:
                result = {"executed": True, "equation": request.name}
        ...
        raise HTTPException(status_code=503, detail="Equation execution service unavailable")
