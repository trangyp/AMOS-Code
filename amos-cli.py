...
def cmd_status(args):
    ...
    try:
        result = subprocess.run(
            ["docker-compose", "ps"], capture_output=True, text=True, cwd=Path(__file__).parent
        )
...
def cmd_deploy(args):
    ...
    result = subprocess.run(["docker-compose", "up", "-d"], cwd=Path(__file__).parent)
    if result.returncode == 0:
...
def cmd_backup(args):
    ...
    result = subprocess.run(
        ["tar", "-czf", str(backup_file), "data/", "amos-memory/", "amos-logs/"],
...
def cmd_key_generate(args):
    ...
    try:
        r = requests.post(
            f"{BASE_URL}/admin/keys", headers={"X-Master-Key": MASTER_KEY}, timeout=10
        )
        if r.status_code == 200:
            data = r.json()
...
def cmd_analytics(args):
    ...
    try:
        r = requests.get(
            f"{BASE_URL}/admin/stats", headers={"X-Master-Key": MASTER_KEY}, timeout=10
        )
        if r.status_code == 200:
            data = r.json()
            print("\n" + "=" * 50)
...
def main():
    parser = argparse.ArgumentParser(description="AMOS Brain CLI - Unified Management Tool")
