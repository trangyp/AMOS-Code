...
class AMOSIntegrationTestSuite:
    ...
    def _test_ledger(self) -> Tuple[bool, str, dict]:
        ...
        try:
            ...
            return (
                True,
                "",
                {"entries": len(ledger._entries), "hash": entry.hash[:8] if entry else "none"},
            )
    ...
    def _test_verification(self) -> Tuple[bool, str, dict]:
        ...
        try:
            ...
            return (
                result.valid,
                "",
                {"proof": result.proof_hash[:8] if result.proof_hash else "none"},
            )
        except Exception as e:
            return False, str(e), {}
    ...
    def _test_multi_engine(self) -> Tuple[bool, str, dict]:
        ...
        try:
            ...
            tasks = ["Consulting task", "Coding task", "Legal task", "Vietnam task"]
    ...
    def _run_test(self, name: str, test_func):
        ...
        try:
            ...
            result = TestResult(
                name=name, passed=passed, duration_ms=duration, error=error, details=details
            )
            ...
            duration = int((time.time() - start) * 1000)
            result = TestResult(
                name=name, passed=False, duration_ms=duration, error=str(e), details={}
            )
            self.results.append(result)
            self.tests_run += 1
            self.tests_failed += 1
