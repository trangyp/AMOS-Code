...
from auth_middleware import _api_keys, generate_api_key, get_key_stats
...
def require_master_key(f):
    ...
    def decorated(*args, **kwargs):
        provided = request.headers.get("X-Master-Key") or request.headers.get(
            "Authorization", ""
        ).replace("Bearer ", "")
...
def create_key():
    ...
    return jsonify(
        {"api_key": key, "message": "Store this key securely. It cannot be retrieved again."}
    )
...
def list_keys():
    ...
    for key_hash, data in _api_keys.items():
        keys_info.append(
            {"id": key_hash[:16] + "...", "active": data["active"], "requests": data["requests"]}
        )
...
def get_stats():
    ...
    return jsonify(
        {"authentication": auth_stats, "rate_limiting": rate_stats, "domain": "neurosyncai.tech"}
    )
