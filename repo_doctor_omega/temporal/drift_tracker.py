    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "destabilizing_commits": [
                {"commit": c, "drift": d}
                for c, d in self.destabilizing_commits
            ],
        }
...
class DriftTracker:
    ...
    def analyze_drift(
        ...
        destabilizing = [
            (d.to_commit, d.drift_norm)
            for d in drifts
            if d.drift_norm > threshold
        ]
    ...
    def _analyze_trends(self, history: list[CommitState]) -> list[DriftTrend]:
        ...
        for key in sorted(all_keys):
            ...
            values = [
                state.state_vector.get(key, 1.0)
                for state in history
            ]
    ...
    def get_drift_timeseries(
        ...
        return [
            (state.commit_hash, state.state_vector.get(subsystem, 1.0))
            for state in history
        ]
