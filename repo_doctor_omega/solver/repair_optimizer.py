...
class RepairOptimizer:
    ...
    def _generate_candidates(self, violations: list[Any]) -> list[RepairCandidate]:
        ...
        for v in violations:
            ...
            candidate = self._violation_to_candidate(
                invariant, message, location, remediation
            )
            if candidate:
    ...
    def _describe_action(self, candidate: RepairCandidate) -> str:
        ...
        return descriptions.get(
            candidate.action,
            f"{candidate.action} on {candidate.target}"
        )
    ...
    def _identify_risks(self, actions: list[RepairAction]) -> list[str]:
        ...
        if high_blast:
            risks.append(
                f"High blast radius: {len(high_blast)} actions may affect "
                f"unrelated subsystems"
            )
        ...
        if total_cost > 20:
            risks.append(
                f"High repair cost ({total_cost:.1f}): Consider refactoring "
                f"over incremental fixes"
            )
