...
class APISubstrate:
    ...
    def compute_api_commutator(
        self, 
        public_api: list[APISymbol], 
        runtime_api: list[APISymbol]
    ) -> list[APIDiscrepancy]:
        """
        ...
        for symbol in runtime_api:
            if (symbol.is_exported and 
                symbol.name not in exported_names and
    ...
    def get_summary(self, results: dict[str, list[APIDiscrepancy]]) -> dict[str, Any]:
        ...
        missing_exports = sum(
            ...
            for x in d if x.kind == "missing_export"
        )
        undocumented = sum(
            1 for d in results.values() 
            for x in d if x.kind == "undocumented"
        )
