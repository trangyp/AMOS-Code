...
class AMOSUnifiedTest:
    ...
    def test_omega_runtime(self) -> TestSuite:
        ...
        try:
            ...
            suite.cases.append(TestCase(
                ...
                level=TestLevel.UNIT,
                result=TestResult.PASS if Substrate.CLASSICAL in substrates else TestResult.FAIL,
                duration_ms=1.0,
    ...
    def test_end_to_end(self) -> TestSuite:
        ...
        try:
            ...
            coherence_result = coh_omega.process_message(
                "I need help making a complex decision",
                validate=True
            )
    ...
    def get_exit_code(self) -> int:
        ...
        total_failed = sum(s.failed for s in self.suites)
        total_errors = sum(
            sum(1 for c in s.cases if c.result == TestResult.ERROR)
            for s in self.suites
        )
