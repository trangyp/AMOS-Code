            ...
            try:
                ...
                for node in ast.walk(tree):
                    ...
                    if isinstance(node, ast.FunctionDef):
                        ...
                        if any(decorator_name(node) in ['tool', 'Tool', 'register'] for node in node.decorator_list):
                            tool = DiscoveredTool(
                                name=node.name,
                                file_path=str(py_file.relative_to(self.root)),
                                function=node.name
    ...
    def generate_inventory(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "tools": {
                ...
                "list": [
                    {"name": t.name, "file": t.file_path}
                    for t in self.tools[:30]
                ]
            },
            "knowledge_bases": {
            ...
            "demos": {
                "count": len(self.demos),
                "list": [str(d) for d in self.demos]
            }
        }
...
def main():
    ...
    if args.quick:
        ...
        result = subprocess.run(
            ...
            capture_output=True, text=True, cwd=str(inventory.root)
        )
