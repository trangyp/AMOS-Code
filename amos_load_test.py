...
class AMOSLoadTester:
    ...
    async def test_think_endpoint(self, session: aiohttp.ClientSession) -> bool:
        ...
        try:
            payload = {
                "query": "What is the load test query?",
                "domain": "software"
            }
    ...
    async def test_decide_endpoint(self, session: aiohttp.ClientSession) -> bool:
        ...
        try:
            payload = {
                "question": "Should we run a load test?",
                "options": ["Yes", "No"]
            }
            async with session.post(
    ...
    async def run_constant_load(self, duration: int, concurrency: int, endpoint: str = "mixed"):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            # Start workers
            tasks = [
                self._worker(session, duration, endpoint)
    ...
    async def run_ramp_up_test(self, max_concurrency: int = 50, step: int = 5):
        ...
        for concurrency in range(step, max_concurrency + 1, step):
            ...
            print(f"   RPS: {self.results.requests_per_second:.1f} | "
    ...
    def print_report(self):
        ...
        print(f"   Min Response: {min(self.results.response_times):.1f}ms" if self.results.response_times else "   N/A")
        print(f"   Max Response: {max(self.results.response_times):.1f}ms" if self.results.response_times else "   N/A")
...
def main():
    ...
    parser.add_argument('--endpoint', default='mixed', 
    ...
    try:
        if args.ramp_up:
            ...
            asyncio.run(tester.run_constant_load(
