...
def cmd_process(args):
    ...
    print("Rule of 4 (Quadrant Analysis):")
...
def cmd_demo(args):
    ...
    for i, task in enumerate(demo_tasks, 1):
        ...
        print(
            f"Rule of 2: {'✓' if result.rule_of_two_check['compliant'] else '✗'} "
            + f"({result.rule_of_two_check['perspectives_checked']} perspectives)"
        )
        print(
            f"Rule of 4: {'✓' if result.rule_of_four_check['compliant'] else '✗'} "
...
def cmd_laws(args):
    ...
    print(
        f"[L4] Structural Integrity: {'✓' if ok else '✗'} "
        + (
