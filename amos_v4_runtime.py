...
class WorldModelEngineV4:
    ...
    def ingest_signal(self, signal: Signal):
        ...
        if len(self.source_reliability[signal.source]) > 20:
            self.source_reliability[signal.source] = \
                self.source_reliability[signal.source][-20:]
    
    def update_reliability_scores(self, source: str, actual_outcome: float, 
                                   predicted: float):
    ...
    def fuse_signals(self, topic: str) -> Dict:
        ...
        relevant_signals = [
            ...
            if topic in str(s.data) or topic in s.source
        ]
        ...
        weighted_value = sum(
            s.data.get(topic, 0.5) * s.effective_weight() 
    ...
    def get_market_state(self) -> Dict[str, Dict]:
        ...
        topics = ['opportunity_index', 'competition_pressure', 'resource_availability', 
...
class AllocationPolicy:
    ...
    resource_type_weights: Dict[str, float] = field(default_factory=lambda: {
        'time': 0.4, 'capital': 0.3, 'attention': 0.2, 'compute': 0.1
    })
...
class AdaptiveResourceAllocator:
    ...
    def allocate_with_learning(self, demands: List[Dict], 
                               world_state: Dict) -> Dict[str, Dict[str, float]]:
    ...
    def _exploitative_allocation(self, demands: List[Dict], 
        ...
        for demand, score in scored_demands:
            ...
            capital_alloc = min(
                ...
                remaining['capital'] * self.policy.resource_type_weights['capital'] * min(score, 1.0)
            )
    ...
    def _calculate_resource_efficiency(self, demand: Dict) -> float:
        ...
        total_cost = (demand.get('time_needed', 0) * 10 +  # Time value
                     demand.get('capital_needed', 0))
    ...
    def learn_from_outcome(self, goal_id: str, allocated_resources: Dict, 
                          actual_return: float, expected_return: float):
        ...
        for goal_id, returns in self.returns_history.items():
            if returns:
                insights['goal_performance'][goal_id] = {
                    ...
                    'trend': 'improving' if len(returns) > 1 and 
                            returns[-1] > statistics.mean(returns[:-1]) else 'stable',
                    'samples': len(returns)
                }
...
class IdentityPreservingEconomics:
    ...
    def _update_identity_fingerprint(self):
        ...
        self.identity_state['identity_fingerprint'] = \
            hashlib.sha256(identity_str.encode()).hexdigest()[:16]
    ...
    def evaluate_action_identity_impact(self, action: Dict) -> Dict:
        ...
        allowed = (
            drift_score <= self.constraints.max_compromise_per_action and
            projected_total <= self.constraints.cumulative_drift_limit
        )
    ...
    def _decay_drift(self):
        ...
        recent_compromises = [
            c for c in self.identity_state['compromise_history']
    ...
    def get_identity_status(self) -> Dict:
        ...
        return {
            ...
            'health_percentage': max(0, 1.0 - (self.identity_state['drift_score'] / 
                                               self.constraints.cumulative_drift_limit)),
...
class FeedbackCompressor:
    ...
    def register_leading_indicator(self, action_type: str, 
                                   indicator_fn: Callable[[Dict], float]):
    ...
    def get_compressed_feedback(self, action: Dict, 
                                time_since_action: timedelta) -> Dict:
        """
    ...
    def _get_surrogate_feedback(self, action: Dict) -> Dict:
        ...
        similar_outcomes = [
            p for p in self.partial_outcomes
...
class AMOSv4ProductionRuntime:
    ...
    def __init__(self, name: str = "AMOS_Production"):
        ...
        try:
            from amos_v4 import AMOSv4, PersistenceManager, EconomicEngine, GoalPortfolio
    ...
    def cycle(self, goals: List[Dict], world_signals: List[Signal]) -> Dict:
        ...
        viable_actions = [
            a for a in evaluated_actions 
            if a['identity_allowed'] and a['combined_score'] > 0
        ]
        ...
        allocations = self.adaptive_allocator.allocate_with_learning(
            viable_actions, 
            self.world_model.get_market_state()
        )
        ...
        feedback = self.feedback_compressor.get_compressed_feedback(
            best_action, time_since
        )
        
        # 9. Update systems
        learning_updates = self._update_systems(
            best_action, execution_result, feedback
        )
        ...
        for goal in goals:
            ...
            for variant in ['minimal', 'balanced', 'aggressive']:
                ...
                actions.append({
                    ...
                    'expected_return': goal['expected_value'] * (0.8 if variant == 'minimal' else 1.0 if variant == 'balanced' else 1.2),
                    'time_needed': goal.get('resource_cost', {}).get('time', 20) * resource_mult,
                    'capital_needed': goal.get('resource_cost', {}).get('capital', 0) * resource_mult,
                    'risk_score': goal.get('risk_score', 0.1) * (1.2 if variant == 'aggressive' else 1.0)
                })
    ...
    def _update_systems(self, action: Dict, execution: Dict, 
                       feedback: Dict) -> Dict:
...
def demo_production_v4():
    ...
    signals = [
        ...
        Signal('market_data', {'opportunity_index': 0.75}, datetime.utcnow(), 0.8, 0.8, latency_hours=1),
    ...
    print("\n[2] Goal Portfolio")
    goals = [
        {'id': 'product', 'name': 'Build Product', 'type': 'product',
         'expected_value': 200, 'resource_cost': {'time': 40, 'capital': 500},
         'risk_score': 0.3},
        {'id': 'skill', 'name': 'Learn Skill', 'type': 'skill',
         'expected_value': 100, 'resource_cost': {'time': 20, 'capital': 100},
         'risk_score': 0.1},
        {'id': 'freelance', 'name': 'Freelance Work', 'type': 'freelance',
         'expected_value': 150, 'resource_cost': {'time': 30, 'capital': 0},
         'risk_score': 0.05}
    ]
