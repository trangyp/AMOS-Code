...
class MemoryEntry:
    ...
    entry_id: str = field(default_factory=lambda: hashlib.md5(
        datetime.utcnow().isoformat().encode()).hexdigest()[:12])
...
class EpisodicMemory:
    ...
    def record_episode(self, event: str, context: Dict, outcome: Optional[str] = None) -> MemoryEntry:
...
class ProceduralMemory:
    ...
    def execute_procedure(self, name: str, context: Dict) -> Dict:
        ...
        if missing:
            return {
                'success': False,
                'error': f'Missing preconditions: {missing}'
            }
...
class AMOSMemory:
    ...
    def retrieve(self, query: str, memory_type: Optional[str] = None, n: int = 3) -> List[MemoryEntry]:
        """
        Retrieve(q) = argmax_m [Similarity(q,m) · Relevance(m) · Freshness(m)]
        """
        candidates = []
...
def demo_memory_system():
    ...
    result = memory.procedural.execute_procedure("retrieve_memory", {'satisfied': ['query_formulated']})
    print(f"  ✓ Learned procedure: retrieve_memory")
    ...
    memory.self.record_self_event("capability_added", "Can now store and retrieve memories", significance=0.85)
