...
class BrainTaskProcessor:
    ...
    def process(self, task: str, context: dict | None = None) -> TaskResult:
        ...
        reasoning_steps = self._build_reasoning_chain(task, perspectives, quadrants, kernel_names)

        # 5. Generate output
        output = self._generate_output(task, reasoning_steps, perspectives, quadrants)
    ...
    def _build_reasoning_chain(
        self, task: str, perspectives: list[dict], quadrants: dict, kernels: list[str]
    ) -> list[str]:
    ...
    def _generate_output(
        self, task: str, reasoning_steps: list[str], perspectives: list[dict], quadrants: dict
    ) -> str:
        ...
        output_lines.extend(
            [
                "",
                "### Assessment",
                f"- Rule of 2 compliance: {len(perspectives)} perspectives ✓"
                if len(perspectives) >= 2
                else f"- Rule of 2: Only {len(perspectives)} perspective (consider alternative view)",
                f"- Rule of 4 compliance: {len(quadrants)} quadrants ✓"
                if len(quadrants) >= 4
                else f"- Rule of 4: {len(quadrants)} quadrants",
            ]
        )
    ...
    def get_brain_status(self) -> dict:
        ...
        return {
            ...
            "global_laws": len(self.brain._config.global_laws)
            if self.brain and self.brain._config
            else 0,
        }
