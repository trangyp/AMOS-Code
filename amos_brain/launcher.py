...
Usage:
    amos-brain  # Entry point after pip install
    python -m amos_brain.launcher
"""
...
def run_tutorial():
    ...
    print()
    try:
        # Use entry point if available, fallback to module execution
        result = subprocess.run(["amos-tutorial"], capture_output=True, text=True)
...
def run_cli():
    ...
    try:
        ...
        result = subprocess.run(["amos-cli"], capture_output=True, text=True)
        if result.returncode != 0:
            subprocess.run([sys.executable, "-m", "amos_brain.cli"])
    except FileNotFoundError:
...
def run_cookbook_demo() -> None:
    ...
    print()
    # Try to run demos from installed package
    try:
        from amos_brain.cookbook import run_cookbook_demo as _run_demo

        _run_demo()
    except ImportError:
        print(clr("Cookbook not available in installed package.", "yellow"))
        print("To run demos, execute from source directory:")
        print("  python -m amos_brain.cookbook")
    print()
    input(clr("[Press Enter to return to menu...]", "yellow"))
