...
class GlobalLaws:
    ...
    def _is_contradictory(self, stmt1: str, stmt2: str) -> bool:
        ...
        stmt1_has_neg = any(
            stmt1.lower().startswith(n) or f" {n}" in stmt1.lower() for n in negations
        )
        stmt2_has_neg = any(
    ...
    def validate_action(self, action: str) -> LawCheckResult:
        ...
        Returns:
            LawCheckResult with compliance status and violations
        """
        violations = []

        # Check L1 constraints
        l1_ok, l1_msg = self.check_l1_constraint(action)
        if not l1_ok:
            violations.append(l1_msg)

        # Check L5 communication
        l5_ok, l5_violations = self.l5_communication_check(action)
        if not l5_ok:
            violations.extend(l5_violations)

        return LawCheckResult(compliant=len(violations) == 0, violations=violations)
