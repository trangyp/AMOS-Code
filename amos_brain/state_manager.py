...
class WorkflowSession:
    ...
    def add_step(self, step: ReasoningStep):
        ...
        quadrants = (
            step.input_context.get("quadrants_checked") or step.input_context.get("quadrants") or []
        )
...
class CognitiveStateManager:
    ...
    def record_reasoning_step(
        ...
        if not session:
            ...
            session_id = self.start_session(
                goal="Auto-generated from reasoning step", domain="general"
            )
    ...
    def get_session_context(self, session_id: str  = None) -> dict:
        ...
        return {
            ...
            "latest_confidence": (
                session.reasoning_chain[-1].confidence if session.reasoning_chain else "unknown"
            ),
        }
    ...
    def replay_reasoning(self, session_id: str  = None, format_type: str = "text") -> str:
        ...
        if format_type == "json":
            return json.dumps(
                {"session": asdict(session), "summary": self.get_session_context(session_id)},
                indent=2,
                default=str,
            )
    ...
    def load_session(self, session_id: str) -> Optional[WorkflowSession]:
        ...
        session.reasoning_chain = [
            ReasoningStep(**step_data) for step_data in data.get("reasoning_chain", [])
        ]
