...
def print_stats(explorer: KnowledgeExplorer):
    ...
    print(f"\n{'='*70}")
    print(f"  AMOS KNOWLEDGE BASE STATISTICS")
    print(f"{'='*70}")
...
def print_recommendations(explorer: KnowledgeExplorer, problem: str):
    ...
    print(f"\n{'='*70}")
    print(f"  ENGINE RECOMMENDATIONS FOR:")
    print(f"  \"{problem}\"")
    print(f"{'='*70}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Explore the AMOS knowledge base"
    )
    parser.add_argument(
        "command",
    ...
    parser.add_argument(
        "query",
        nargs="?",
        help="Search query or problem description"
    )
