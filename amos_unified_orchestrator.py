...
from amos_brain import get_amos_integration, orchestrate_goal
...
class AMOSUnifiedOrchestrator:
    ...
    def _validate_integration(self) -> dict[str, Any]:
        ...
        checks = {
            ...
            "critical_systems": all([
                self.subsystems.get("01_BRAIN", SubsystemInfo("", 0, "", SubsystemStatus.OFFLINE)).loaded,
                self.subsystems.get("06_MUSCLE", SubsystemInfo("", 0, "", SubsystemStatus.OFFLINE)).loaded,
            ]),
        }
    ...
    def _route_task(self, task: str, analysis: dict) -> dict[str, Any]:
        ...
        for subsystem, keywords in routing_rules.items():
            if any(kw in task_lower for kw in keywords):
                if self.subsystems.get(subsystem, SubsystemInfo("", 0, "", SubsystemStatus.OFFLINE)).loaded:
    ...
    def get_status(self) -> dict[str, Any]:
        ...
        return {
            "orchestrator": {
                ...
                "health_percentage": (self.state.subsystems_online / self.state.subsystems_total * 100) if self.state.subsystems_total > 0 else 0,
    ...
    def print_dashboard(self):
        ...
        print(f"  Subsystems: {orch['subsystems_online']}/{orch['subsystems_total']} online ({orch['health_percentage']:.0f}%)")
