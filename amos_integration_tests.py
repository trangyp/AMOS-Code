...
class AMOSIntegrationTests:
    ...
    def _test_new_tools(self) -> None:
        ...
        tools_to_test = [
            ...
            (
                "AMOSCode",
                {"layer": "backend", "function_name": "test_func", "description": "Test function"},
            ),
        ...
        for tool_name, params in tools_to_test:
            ...
            self.results.append(
                TestResult(
                    name=f"tool_{tool_name}", passed=passed, duration_ms=duration, error=error
                )
            )
    ...
    def _test_local_runtime(self) -> None:
        ...
        duration = (time.time() - start) * 1000
        self.results.append(
            TestResult(name="local_model_backend", passed=passed, duration_ms=duration, error=error)
        )
        ...
        duration = (time.time() - start) * 1000
        self.results.append(
            TestResult(name="local_runtime", passed=passed, duration_ms=duration, error=error)
        )
        ...
        duration = (time.time() - start) * 1000
        self.results.append(
            TestResult(name="metrics_collector", passed=passed, duration_ms=duration, error=error)
        )
    ...
    def _test_coherence_bridge(self) -> None:
        ...
        duration = (time.time() - start) * 1000
        self.results.append(
            TestResult(name="coherence_bridge", passed=passed, duration_ms=duration, error=error)
        )
    ...
    def _generate_report(self) -> Dict[str, any]:
        ...
        print("  2. Ensure all lazy imports resolve correctly")
        print("  3. Verify local runtime dependencies")
        print("  4. Test coherence bridge with real data")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="AMOS Integration Test Suite")
    parser.add_argument("--tools", action="store_true", help="Test tools only")
    parser.add_argument("--runtime", action="store_true", help="Test runtime only")
    parser.add_argument("--coherence", action="store_true", help="Test coherence only")
