...
class IntegrationConfig:
    ...
    # Runner config
    organism_config: OrganismConfig = field(
        default_factory=OrganismConfig
    )
...
class AMOSOrganismIntegration:
    ...
    async def run_cycle(self) -> CycleMetrics:
        ...
        try:
            ...
            if (self.ethics and 
                self._cycle_count % self.config.ethics_check_frequency == 0):
    ...
    async def run(self, duration_seconds: Optional[float] = None) -> None:
        ...
        print(f"[Integration] Starting runtime...")
        self._running = True
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "runner_state": (
                self.runner.state.name if self.runner else "none"
            ),
            "components": {
    ...
    def get_health_report(self) -> str:
        ...
        if self._metrics:
            ...
            for m in self._metrics[-5:]:
                ...
                lines.append(
                    f"  {status} Cycle {m.cycle_id}: {m.duration_ms:.1f}ms"
                )
