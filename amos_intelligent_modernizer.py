...
class IntelligentModernizer:
    ...
    async def analyze_repository(self) -> List[FileAnalysis]:
        ...
        py_files = [
            f for f in py_files
            if not any(part.startswith(".") or part in ["venv", "__pycache__", "node_modules"]
                      for part in f.parts)
        ]
    ...
    def create_priority_batches(self) -> List[ModernizationBatch]:
        ...
        core_files = [
            a.path for a in self.analyzed_files
        ...
        organism_files = [
            a.path for a in self.analyzed_files
            if "AMOS_ORGANISM_OS/" in str(a.path)
        ]
        ...
        analyzed_paths = set(core_files + backend_files + organism_files +
                           repo_doctor_files + integration_files)
        remaining_files = [
            a.path for a in self.analyzed_files
            if a.path not in analyzed_paths
        ]
    ...
    async def run_batch(self, batch: ModernizationBatch, dry_run: bool = True) -> Dict[str, Any]:
        ...
        print(f"  ✅ Batch complete: {results['files_modified']}/{results['files_processed']} files modified")
        print(f"     Total changes: {results['total_changes']}")

        return results

    async def _modernize_file(self, filepath: Path, patterns: List[str], dry_run: bool) -> Dict[str, Any]:
    ...
    async def generate_report(self) -> Dict[str, Any]:
        ...
        self.report = {
            ...
            "summary": {
                "files_with_datetime_issues": sum(1 for a in self.analyzed_files if a.has_datetime_issues),
                "files_with_typing_issues": sum(1 for a in self.analyzed_files if a.has_typing_issues),
                "files_with_async_issues": sum(1 for a in self.analyzed_files if a.has_async_issues),
            ...
            "top_files_by_issues": [
                {"path": str(a.path), "issues": a.issue_count}
                for a in self.analyzed_files[:20]
            ]
        }
        return self.report
...
async def main():
    ...
    parser.add_argument("--report", type=str, default="modernization_report.json",
