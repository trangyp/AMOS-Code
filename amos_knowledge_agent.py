...
class KnowledgeAgent:
    ...
    def _assess_complexity(self, goal: str) -> str:
        ...
        complexity_keywords = [
            "design", "create", "build", "implement", "system",
            "framework", "architecture", "strategy", "comprehensive"
        ]
    ...
    def create_plan(self, goal_analysis: dict) -> list[dict]:
        ...
        if complexity == "low":
            ...
            steps = [
                ...
                {"step": 5, "action": f"Apply advanced {domain} principles", "knowledge_check": True},
    ...
    def execute_step(self, step: dict, goal: str, context: dict) -> dict:
        ...
        if step.get("knowledge_check") and self.reasoning:
            ...
            if relevant:
                ...
                result["output"] = f"Executed with knowledge from {top_match.engine_key}: {top_match.excerpt[:100]}..."
            else:
                result["output"] = f"Executed step using general {context.get('domain', 'domain')} principles"
        else:
